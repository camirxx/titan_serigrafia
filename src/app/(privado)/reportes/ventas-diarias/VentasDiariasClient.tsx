...
export default function VentasDiariasPage() {
  ...
  async function buscar() {
    ...
    try {
      let query = supabase.from('ventas_diarias_view').select('*').order('fecha', { ascending: false }).limit(365)
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-4 bg-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div className="flex items-end">
          <button onClick={exportCSV} className="w-full h-10 rounded border" disabled={!rows.length}>
            Exportar CSV
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="border rounded p-3">
          ...
          <div className="text-2xl font-semibold">${totalPeriodo.toLocaleString()}</div>
        </div>
      </div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-t">
                ...
                <td className="p-2 text-right">${Number(r.total_vendido).toLocaleString()}</td>
              </tr>
            ))}
            {!rows.length && !loading && <tr><td colSpan={3} className="p-3 text-center text-gray-500">Sin datos</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
